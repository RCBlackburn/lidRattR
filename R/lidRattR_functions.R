...
vox_mt2 <- function(vox, res, max_z)
{
  ...
  fullvox$SVi_D[is.na(fullvox$SVi_D)] <- 0
  ...
  points_am <- suppressWarnings(as.data.table(lapply(points_am,  function(x) as.numeric(unlist(x)))))
  ...
  points_ab <- suppressWarnings(as.data.table(lapply(points_ab,  function(x) as.numeric(unlist(x)))))
  ...
  sv_sum <- fullvox[, .(sv_sum = sum(SVi_D)), by = "id_xy"]
  fullvox <- merge(fullvox, sv_sum, "id_xy")
  fullvox$FR_SVi = fullvox$SVi_D/fullvox$sv_sum
  ...
  fullvox$FR_Di = fullvox$npoints_above/fullvox$sv_sum
  fullvox[is.na(fullvox$FR_Di)]$FR_Di <- 0

  ### sub-voxel maximums
  maxSVM <- fullvox[, .SD[which.max(SVi_D)], by = "id_xy"]
  ...
  metrics = data.table(
    ...
    PDi_below = fullvox$npoints_below,
    PDi_above = fullvox$npoints_above,
    SV_sum = fullvox$sv_sum,
    SVM_md_above = fullvox$pa_med,
    SVM = fullvox$SVM,
    SVM_md = fullvox$SVM_m
...
vox_sum <- function(SVi_D,FRDi, FRSVi, PDi_below, PDi_above, SV_sum, SVM_md_above, SVM_md, SVM, vox_res){
  ...
  las_vox <- las_vox[,-c("SV_sum", "SVM_md", "SVM", "SVM_md_above")]
...
#' for the entir point cloud including the mean, median, variance, standard deviation, coefficient of variation,
...
voxel_summary <- function(las, vox_res = vox_res, max_z ){
  ...
  vox_sum(out$SVi_D, out$FRDi, out$FRSVi, out$PDi_below, out$PDi_above, out$SV_sum, out$SVM_md_above,
          out$SVM_md, out$SVM, vox_res[1])
}
...
voxel_raster <- function(las, vox_res = vox_res, rast_res =  rast_res, max_z ){
  ...
  raster <- pixel_metrics(out,
                          func = vox_sum(SVi_D,FRDi, FRSVi, PDi_below, PDi_above, SV_sum, SVM_md_above, SVM_md, SVM, vox_res = vox_res[1]),
                          res = rast_res)
}
